...
#region Cevap
List<string> cevaplar = new List<string> { "A", "B", "C", "D" };
string dogruCevap = "B";
cevaplar.Remove(dogruCevap);
Random rnd = new Random();
string randomCevap = (cevaplar[rnd.Next(0, 3)]);
cevaplar.Clear();
cevaplar.Add(dogruCevap);
cevaplar.Add(randomCevap);
foreach (var kalanCevap in cevaplar.OrderBy(x => x))
{
